...
function input_barang() {
	...
	$sql = "INSERT INTO barang (kdbrg, brg_nama, brg_stok, brg_harga, brg_kategori, 
		brg_garansi, brg_create, brg_update) VALUES ('$kdbrg','$brg_nama','$brg_stok',
		'$brg_harga', '$brg_kategori','$brg_garansi','$brg_create', '$brg_update')";	
...
function edit_barang()
{
	...
	$brg_create = date('Y-m-d H:i:s');
	$brg_update = date('Y-m-d H:i:s');
	$sql = "UPDATE barang SET brg_nama = '$brg_nama', brg_stok = '$brg_stok',
		brg_harga = '$brg_harga', brg_kategori = '$brg_kategori', brg_garansi = '$brg_garansi'
		WHERE kdbrg = '$kdbrg'";
	$result = mysqli_query($konek,$sql);
	if (!$result) {
		echo mysql_error();
